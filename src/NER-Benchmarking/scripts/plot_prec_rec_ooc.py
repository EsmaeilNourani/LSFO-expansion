...
#     sns.scatterplot(data=df, x='Precision_ooc', y='Recall_ooc', marker='x', color='red', s=100)
    ...
    plt.scatter(df['Precision'], df['Recall'], c=c, marker='o', s=300,label='Without OOC')
    
    scatter=plt.scatter(df['Precision_ooc_as_LSF'], df['Recall_ooc_as_LSF'], c=c, marker='x', s=300,label='OOC as LSF')
  ...
  # Create custom legend handles and labels for "Without OOC"
    ...
    handles, labels = plt.gca().get_legend().legendHandles, [ "Without OOC", "OOC as LSF" ]
    ...
    plt.savefig(options.output_file,bbox_inches='tight',dpi=300)
    plt.show()
