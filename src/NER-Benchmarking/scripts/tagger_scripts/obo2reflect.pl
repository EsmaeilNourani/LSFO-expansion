...
while (<OBO>) {
    ...
    while (<OBO>) {
        ...
        elsif ((/^(name): +(.+)/ or /^(synonym): "(.+?)"/) and not (/" BROAD/)) {
            my $field = $1;
            my $name = $2;
            $name =~ s/\t/ /g;
            if (not exists $id_filter{$id} and not (exists $id_name_filter{$id} and exists $id_name_filter{$id}{$name}) and length($name) >= 3 and not $name =~ / antibody$/) {
                my $priority = 1;
                $priority = 3 if $field ne "name";
		$priority = 4 if /" NARROW/;
		$priority = 5 if /" RELATED/;
...
sub print_children {
    my $current = $_[0];
    foreach my $parent (@parents) {
        if (not exists $id_serial{$parent}) {
            $serial++;
            $id_serial{$parent} = $serial;
        }
...
if (-e $extranamesfile) {
	...
	$priority = 5 unless defined $priority;
        ...
        print NAMES $id_serial{$id}, "\t", $name, "\t", $priority, "\n" unless exists $id_filter{$id} or exists $id_name_filter{$id} and exists $id_name_filter{$id}{$name};
    }
    close TSV;
}
...
my %serial_root = ();
foreach my $parent (keys %parent_child) {
    $serial_root{$parent} = 1 unless exists $serial_root{$parent};
    foreach my $child (keys %{$parent_child{$parent}}) {
        $serial_root{$child} = 0;
    }
}

foreach my $serial (keys %serial_root) {
    print_children($serial) if $serial_root{$serial};
}
